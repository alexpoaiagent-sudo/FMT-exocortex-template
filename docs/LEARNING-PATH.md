...
> **Терминология:** IWE = интеллектуальная рабочая среда (то, что ты используешь). Экзокортекс = инфраструктурный стек (Git, MCP, агенты), на котором построена IWE.
...
### 1.2. Четыре компонента

После установки у тебя работает система из 4 компонентов:

| Компонент | Что делает | Где живёт |
|-----------|-----------|-----------|
| **CLAUDE.md** | Правила для Claude Code: как открывать сессию, фиксировать знания, закрывать | `~/Github/CLAUDE.md` |
| **memory/** | Оперативная память: задачи недели, различения, чеклисты, SOTA | `~/.claude/projects/.../memory/` |
| **Стратег** | Автоматический агент: утренний план, вечернее ревью, недельная стратегия | `agents/strategist/` (в шаблоне) |
| **DS-strategy/** | Стратегический хаб: планы недель, отчёты, входящие заметки | `~/Github/DS-strategy/` (отдельный репо) |
...
IWE существует не изолированно — она часть 4-контурной системы:
...
3. Жёсткие различения (§ 3.2) — 20 мин
4. 5 типов репозиториев (§ 4.1) — 15 мин
...
4. Стратег и планирование (§ 5.5) — 20 мин
