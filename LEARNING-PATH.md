...
1. **Новичок:** Начни с разделов 1-3 (фундамент, уровни системы, репозитории, протоколы). Это ~3 часа.
...
22 пары понятий, которые **нельзя путать**. Путаница в этих парах — главный источник ошибок.
...
| 22 | Platform ≠ IWE Template ≠ Personal IWE | 4 уровня системы (§ 1.4) |

**Где изучить:**
- [memory/hard-distinctions.md](./memory/hard-distinctions.md) — все 22 пары с примерами и тестами

### 1.4. Уровни системы (Platform Levels)

IWE существует не изолированно — она часть 4-уровневой системы. Понимание уровней помогает видеть, где ты находишься и что тебе доступно.

```
L1: Ecosystem    — вся система: платформа + сообщество + все IWE пользователей + МИМ
  L2: Platform   — инфраструктура и сервисы (бот, MCP, Knowledge Index, Pack-репо)
    L3: Template — этот шаблон (CLAUDE.md + memory/ + стратег-агент + DS-strategy/)
      L4: Personal IWE — твой экземпляр (настроенный под тебя, с личными Pack и данными)
```

**Как это работает:**

| Уровень | Что для тебя | Пример | Как обновляется |
|---------|-------------|--------|-----------------|
| **L1: Ecosystem** | Сообщество, семинары, контент | systemsworld.club, Telegram-каналы | Ты участвуешь |
| **L2: Platform** | Сервисы, к которым ты подключаешься | Бот @aist_me_bot, Knowledge Index | Обновляется разработчиком платформы |
| **L3: Template** | Шаблон, из которого создан твой IWE | Этот репо (FMT-exocortex-template) | `update.sh` — обновляет Platform-space |
| **L4: Personal IWE** | Твоя работа, планы, знания | ~/Github/CLAUDE.md, DS-strategy/ | Только ты (User-space) |

**Путь пользователя через уровни:**

1. Заходишь в бот (L2, Platform) → T1-T3
2. Устанавливаешь экзокортекс из шаблона (L3 → L4) → T4
3. Настраиваешь под себя: личные Pack, DS-strategy, правила (L4)
4. Подключаешь опциональные сервисы (L3 рекомендует, L4 настраивает)

**Опциональные сервисы:**

Шаблон (L3) рекомендует, но не обязывает. Пользователь решает сам.

| Сервис | Тип | Роль | Продукт |
|--------|-----|------|---------|
| WakaTime | Инструмент | Observability работы | Метрики времени по проектам |

**Правило IntegrationGate:** Перед добавлением нового инструмента или агента в свой IWE — определи: (1) тип (инструмент/агент), (2) уровень (L2/L3/L4), (3) роли, (4) продукты, (5) процессы. Это помогает не добавлять лишнего и понимать, как новый компонент встраивается.

**Где изучить:**
- [ONTOLOGY.md](./ONTOLOGY.md) § «Уровни системы» — компактное описание
- [DS-ecosystem-development/11-platform-levels.md](https://github.com/TserenTserenov/DS-ecosystem-development) — полная архитектурная модель с маппингом на ядра
- [memory/hard-distinctions.md](./memory/hard-distinctions.md) § 22 — тесты различения уровней
...
- Cross-Pack Role Index (35 ролей: 21 DP + 7 EDU + 4 PD + 3 MIM): [DP.AGENT.001 §3.4](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AGENT.001-ai-agents.md)
- Мета-роли владельца (M1-M4: разработчик экосистемы/платформы/шаблона, пользователь IWE): [DP.AGENT.001 §3.3](https://github.com/TserenTserenov/PACK-digital-platform/blob/main/pack/digital-platform/02-domain-entities/DP.AGENT.001-ai-agents.md)
...
**Маппинг тиров на уровни системы (§ 1.4):**

| Тир | Уровни | Что доступно |
|-----|--------|-------------|
...
| На каком уровне я нахожусь? | L4 (Personal IWE), если T4+. L2 (Platform), если T1-T3 | § 1.4 |
| Куда добавить новый инструмент? | Определи уровень: L2 (платформа) / L3 (шаблон) / L4 (личный) | [CLAUDE.md](./CLAUDE.md) Pre-action Gates (IntegrationGate) |
...
1. Прочитай уровни системы (§ 1.4) — 10 мин (пойми, где ты в системе)
2. Прочитай жёсткие различения (§ 1.3) — 20 мин
3. Прочитай 5 типов репозиториев (§ 2.1) — 15 мин
4. Прочитай иерархию принципов (§ 1.2) — 15 мин
5. Пробеги FPF reference (§ 8) — 10 мин
